<script lang="ts">
	...
	// schematic fallback when no real Qwen vector is supplied
	const randomFallback = new Array(visibleDimension).fill(0).map(() => Math.random());
	$: drawData = data && data.length ? data : randomFallback;
	...
	function drawCanvas() {
		...
		for (let i = 0; i < height / lineHeight; i++) {
			const value = drawData[i % n]; // tile if the real vector is shorter than the height
