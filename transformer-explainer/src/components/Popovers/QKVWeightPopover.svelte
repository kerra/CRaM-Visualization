...
<WeightPopoverCard id="qkv" title={'Qwen Q/K/V Projections + RoPE'} bind:isAnimationActive {timeline}>
	<div class="weight-popover-content flex items-center justify-start">
		...
		<div class="matrix flex flex-col items-center">
			<div class="title flex items-center gap-1">
				Q/K/V projection weights<HelpPopover id="qkv-weights" 
					>{`Qwen uses separate learned q_proj, k_proj, and v_proj layers. \nRotary position embeddings are then applied to Q and K.`}</HelpPopover
				>
			</div>
		...
		<div class="matrix flex flex-col items-center">
			<div class="title flex items-center gap-1">
				Projection biases<HelpPopover id="qkv-bias" 
		...
		<div class="matrix flex flex-col items-center">
			<div class="title">Q · K · V (before RoPE)</div>
			<div class="flex">
				<Matrix
		...
		<div class="first-row flex items-center justify-center gap-1">
			<span class="part1">
				<Katex
					displayMode
					math={`
	(X_{1,1}W_{1,1} + \\cdots + X_{1,${$modelMeta.dimension}}W_{${$modelMeta.dimension},1})`}
				/>
			</span>
			<span class="part2">
				<Katex displayMode math={`+ b_1 = [Q;K;V]_{1,1}`} />
			</span>
		</div>
		<div class="total">
			<Katex
	...
	\\qquad \\widetilde Q=\\operatorname{RoPE}(Q),\\;\\widetilde K=\\operatorname{RoPE}(K)
	`}
			/>
		</div>
	</div>
