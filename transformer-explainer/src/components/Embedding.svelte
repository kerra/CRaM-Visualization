<script lang="ts">
	...
	import { tokenGroupLabels } from '~/utils/tokenGroups';

	// real Qwen token-embedding vectors (normalized for the color scale)
	$: embVecs = $activeTrace?.embedding
		? $activeTrace.embedding.map((v: number[]) => normalizeVector(v))
		: [];
	$: wordLabels = tokenGroupLabels($tokens, $activeTrace?.source_indices);
			...
			<div class="column token-string relative">
				...
				{#each $tokens as token, index}
					<div class="cell" class:last={index === $tokens.length - 1}>
						<span
							class="label"
							title={`${wordLabels[index]?.label || token} · ${wordLabels[index]?.span || 1} token(s)`}
							>{wordLabels[index]?.label || ''}</span
						>
					</div>
				{/each}
			</div>
			...
			<div class="column vectors embedding-column">
				{#each $tokens as token, index}
					<div class={`vector ${embeddingVectorColor}`} class:last={index === $tokens.length - 1}>
						<VectorCanvas active={$blockIdx === 0 && (isHovered || isEmbeddingExpanded)} data={embVecs[index]} />
					</div>
				{/each}
			</div>
