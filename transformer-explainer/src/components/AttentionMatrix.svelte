<script lang="ts">
	...
	$: currentHeadIsSelected =
		Array.isArray(selectedHeadMap[String($blockIdx)]) &&
		selectedHeadMap[String($blockIdx)].includes($attentionHeadIdx);
	...
	const onKeyDownAttention = (e: KeyboardEvent) => {
...
>
	...
	<div
		role="button"
		tabindex="0"
		aria-label={isAttentionExpanded ? 'Attention calculations expanded' : 'Expand attention calculations'}
	...
	>
		...
		>
			...
			<Tooltip class="popover tooltip">
				<Katex math={'\\widetilde{Q}_h\\widetilde{K}_h^T\\quad (\\widetilde{Q},\\widetilde{K}=\\operatorname{RoPE}(Q,K))'}></Katex>
			</Tooltip>
		...
		>
			...
			<TextbookTooltip id="masked-self-attention">
				<div class="matrix-label">
					{$cramEnabled && currentHeadIsSelected ? 'Official CrAM mask' : 'Qwen causal mask'}
				</div>
			</TextbookTooltip>

			<Tooltip class="popover tooltip">
				{#if $cramEnabled && currentHeadIsSelected}
					<Katex math={'M_h^{\\mathrm{CrAM}}=\\operatorname{edit\\_attention\\_mask}(M,\\log\\bar{s},h)'}></Katex>
					<div>Captured after the official CrAM repository hook; this is the mask consumed by Qwen.</div>
				{:else}
					<Katex math={'M=M_{\\mathrm{causal}}'}></Katex>
					<div>Captured from Qwen immediately before self-attention.</div>
				{/if}
			</Tooltip>
			<div class="color-scale">
		...
		>
			...
			<Tooltip class="popover tooltip">
				{#if $cramEnabled && currentHeadIsSelected}
					<Katex
						math={'A_h^{\\mathrm{CrAM}}=\\operatorname{softmax}\\!\\left(\\frac{\\widetilde Q_h\\widetilde K_h^T}{\\sqrt{d_h}}+M_h^{\\mathrm{CrAM}}\\right)'}
					></Katex>
					<div>Attention probabilities returned by Qwen’s actual CrAM-modified forward pass.</div>
				{:else if $cramEnabled}
					<Katex math={'h\\notin H_{\\mathrm{CrAM}}:\\quad A_h=\\operatorname{softmax}(Z_h)'}></Katex>
				{:else}
					<Katex math={'A_h=\\operatorname{softmax}(Z_h)'}></Katex>
				{/if}
			</Tooltip>
			<div class="color-scale">
